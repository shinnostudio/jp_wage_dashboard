...
layer = pdk.Layer(
    ...
    data = df_pref_map,
    opacity = 0.4,                          # 不透明度
    get_position =['lon','lat'],            # ポジションの指定
    threshold = 0.3,                        # ヒートマップのしきい値
    get_weight = '一人あたり賃金（相対値）'     # どの列をヒートマップとして可視化するか
)
...
st.header('集計年別の一人あたりの賃金（万円）の推移')
